        ...
        public ActionResult Index(string searchString, string sortOrder, string edicao, bool finalizado = false)
        {
            if (ADAuthorization.ADAuthenticate())
            {
                ...
                foreach (Genero genero in generos)
                {
                    ...
                    if (!String.IsNullOrEmpty(edicao) && finalizado)
                    {
                        displayCurso.Total = db.Candidatura.Where(c => c.DadosPessoais.Genero == genero.ID && c.Edicao == edicao && c.Certificado != null).Count();
                        displayCurso.Percentagem = Math.Round(((double)(db.Candidatura.Where(c => c.DadosPessoais.Genero == genero.ID && c.Edicao == edicao && c.Certificado != null).Count() / (double)ViewBag.TotalCandidatos) * 100), 2);
                    }
                    else if (finalizado)
                    {
                        displayCurso.Total = db.Candidatura.Where(c => c.DadosPessoais.Genero == genero.ID && c.Certificado != null).Count();
                        displayCurso.Percentagem = Math.Round(((double)(db.Candidatura.Where(c => c.DadosPessoais.Genero == genero.ID && c.Certificado != null).Count() / (double)ViewBag.TotalCandidatos) * 100), 2);
                    }
                    else if (!String.IsNullOrEmpty(edicao))
                    {
